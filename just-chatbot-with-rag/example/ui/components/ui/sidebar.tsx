...
export function SidebarProvider({ children, defaultOpen = true }: { children: React.ReactNode; defaultOpen?: boolean }) {
  ...
  const toggleSidebar = React.useCallback(() => setOpen((v) => {
    const next = !v;
    try { document.cookie = `${COOKIE}=${String(next)}; path=/; max-age=${60 * 60 * 24 * 7}`; } catch {}
    return next;
  }), []);
...
export function SidebarTrigger({ children, className, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { children?: React.ReactNode }) {
  const { toggleSidebar } = useSidebar();
  return (
    <button type="button" onClick={toggleSidebar} className={className} {...props}>
      {children}
